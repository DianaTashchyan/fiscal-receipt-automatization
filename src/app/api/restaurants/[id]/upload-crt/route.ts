...
export async function POST(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    try {
      ...
      // Detect the case where our CSR and private key are a matching pair but the uploaded
      // certificate was signed for a completely different key (e.g. one generated by VCR.am).
      const isVcrLikeCase =
        csrFingerprint !== null &&
        privKeyFingerprint !== null &&
        csrFingerprint === privKeyFingerprint &&
        certFingerprint !== csrFingerprint;

      const errorMessage = isVcrLikeCase
        ? "This certificate was issued for another private key. Generate a new CSR in this system (step 2) and upload the new SRC-issued .crt."
        : `Certificate/key mismatch: ${(e as Error).message}`;
    ...
    // Verify the certificate was also issued for the CSR on file.
    // Normally guaranteed when CERT==KEY and CSR==KEY, but guards against DB state drift.
    if (csrFingerprint !== null && certFingerprint !== csrFingerprint) {
      return NextResponse.json(
        {
          error:
            "This certificate was not issued for the CSR generated in this system. " +
            "Generate a new CSR (step 2), submit it to SRC, and upload the new SRC-issued .crt.",
          diagnosis: {
            fingerprints: {
              CERT:        certFingerprint,
              CSR:         csrFingerprint,
